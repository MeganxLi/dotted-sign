...
const Head = () => {
  ...
  const MenuList = (flat: boolean): JSX.Element[] => routes.map((page, idx) => (
    <li
      key={idx}
  ...
  return (
    <>
      ...
      <ul
        className={`menu absolute left-0 top-0 z-40 hidden w-full flex-col items-center gap-6 bg-white text-black/50 flat:flex ${openMenu ? 'scale-up-top-right pt-24 pb-16' : 'scale-down-top-right origin-top-right'} `}
      >
        {openMenu && (
          <>
            {MenuList(true)}
